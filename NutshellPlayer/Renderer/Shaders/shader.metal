...
// BT.601 full-range YUV to gamma-encoded RGB matrix
// Assumes Y ∈ [0,1], Cb/Cr ∈ [-0.5, 0.5]
constant half3x3 YUV_TO_RGB_BT601 = half3x3(
                                            half3(1.0h,  1.0h, 1.0h),
                                            half3(0.0h, -0.344136h, 1.772000h),
                                            half3(1.402000h, -0.714136h, 0.0h)
                                            );
...
half3 convert_8bit_limited_yuv_bt601(half y_norm, half2 cbcr_norm) {
    half y_8bit = y_norm * 255.0h;
    half2 cbcr_8bit = cbcr_norm * 255.0h;
    half y_full = (y_8bit - 16.0h) / 219.0h;           // 235-16=219
    half2 cbcr_centered = (cbcr_8bit - 128.0h) / 224.0h; // 240-16=224
...
// ---------- BT.601 Full Range Helpers ----------
half3 convert_8bit_full_yuv_bt601(half y_norm, half2 cbcr_norm) {
...
// ========== BT.601 Fragment Shaders ==========

// ---------- BT.601 8-bit Limited ----------
[[fragment]]
half4 fragment_8bit_limited_sbs_bt601(VertexOut in [[stage_in]],
                                      constant uint &viewID [[buffer(0)]],
                                      texture2d<half, access::sample> lumaTexture [[texture(0)]],
                                      texture2d<half, access::sample> chromaTexture [[texture(1)]]) {
    constexpr sampler s(address::clamp_to_edge, filter::linear);
    ...
    half y = lumaTexture.sample(s, uv).r;
    half2 cbcr = chromaTexture.sample(s, uv).rg;
    return half4(convert_8bit_limited_yuv_bt601(y, cbcr), 1.0h);
}
...
// ---------- BT.601 8-bit Full ----------
[[fragment]]
half4 fragment_8bit_full_sbs_bt601(VertexOut in [[stage_in]],
                                   constant uint &viewID [[buffer(0)]],
                                   texture2d<half, access::sample> lumaTexture [[texture(0)]],
                                   texture2d<half, access::sample> chromaTexture [[texture(1)]]) {
    constexpr sampler s(address::clamp_to_edge, filter::linear);
    ...
    half y = lumaTexture.sample(s, uv).r;
    half2 cbcr = chromaTexture.sample(s, uv).rg;
    return half4(convert_8bit_full_yuv_bt601(y, cbcr), 1.0h);
}
